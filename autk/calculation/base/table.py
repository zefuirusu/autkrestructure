...
class ImmortalTable:
    ...
    ImmortalTable.
        ...
        2. key_index and key_name are needed only when you have to combine a few columns as the KEY column.
            Simply loading data of ImmortalTable does not needed key_index and key_name. 
        ...
        All the above types of `self.xlmeta` can be generated by class `autk.meta.pmeta.JsonMeta`;
        Remember to call `self.__clear_temp()` before every calculation.
    '''
    def __init__(
        self,
        ...
        if isinstance(xlmeta,JsonMeta):
            ...
            if isinstance(xlmap,XlMap):
                ...
                print(
                    '[{}]Load blank Table with no `xlmap`.'.format(
                        self.__class__.__name__,
                    )
                )
                self.data=None
    ...
    def xlshape(self)->DataFrame:
        ...
        for xl in self.xlset:
            shape.append([
                xl.name,
                int(xl.show['data_size'][0]),
                int(xl.show['data_size'][1])
            ])
        shape=DataFrame(
            shape,
            columns=[
    ...
    def shape(self):
        ...
        return (
            int(self.xlshape['rows'].sum()),
            int(self.xlshape['columns'].max())
        )
    @property
    def show(self):
        show={
            ...
            "xlmap":{
                str(type(self.xlmap)) if self.xlmap is None else self.xlmap.__class__.__name__:
    ...
    def collect_xl(self):
        ...
        def __single_append(shmeta):
            ...
            if isinstance(self.xlmap,XlMap):
                # ImmortalTable instance has `xlmap`;
                # Load normally.
                print(
                ...
                # ImmortalTable instance does not have `xlmap`;
                # get `xlmap` from DataFrame.
                print(
                    '[{}]Get `xlmap` from DataFrame.'.format(
                        self.__class__.__name__,
                    )
                )
                self.xlmap=XlMap()
            ...
            if self.xlmeta.keep_additional==True:
                ...
                xl.append_col_name('from_sheet')
                xl.data['from_sheet']=sheet_name
                pass
            else:
                print(
                    '[{}] `xlmeta` does not keep additional source-columns.'.format(
                        self.__class__.__name__
                    )
                )
                pass
